...
class ScaledWSConv1d(nn.Conv1d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
        ...
        nn.Conv1d.__init__(self, in_channels, out_channels,
                           ...
                           groups, bias)
        # self.causal_mask = torch.ones_like(self.weight)## i changed this, so the model runs with FP16 checkpoint on CPU
        self.register_buffer("causal_mask", torch.ones_like(self.weight), persistent=False)
    ...
    def get_weight(self):
        #so code does not error out when model is loaded in fp16 on cpu
        mask = self.causal_mask.to(device=self.weight.device, dtype=self.weight.dtype)
...
class UConvBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.spp_dw.append(ConvAct(in_channels, in_channels, kSize=11,
...
class CausalSuDORMRF(nn.Module):
    def __init__(self,
        ...
        # Front end
        # self.encoder = ScaledWSConv1d(
        #     in_channels=in_audio_channels,
        #     out_channels=enc_num_basis,
        #     kernel_size=enc_kernel_size,
        #     stride=enc_kernel_size // 2,
        #     padding=enc_kernel_size // 2,   # <-- change this
        #     bias=False
        # )
        # Front end
    ...
    def pad_to_appropriate_length(self, x):
        ...
        padded_x = torch.zeros(list(x.shape[:-1]) + [values_to_pad], dtype=x.dtype) #change from 32 to be detected incase it is for 16
